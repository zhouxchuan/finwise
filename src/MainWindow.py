from PySide6.QtWidgets import QMainWindow, QDialog, QMessageBox, QSplitter, QWidget, QSizePolicy, QListWidgetItem, QMenu, QTableWidgetItem, QHeaderView
from PySide6.QtCore import Qt, QDate
from PySide6.QtGui import QAction


from utils.mysqldb import MySQLDatabase as MySQLDB
from ui.MainWindow_ui import Ui_mainWindow
from LoginDialog import LoginDialog
from CreateFundAccountDialog import CreateFundAccountDialog
from SyncFundHistoryDialog import SyncFundHistoryDialog
from SetupFundAccountDialog import SetupFundAccountDialog
from SetupOptionsDialog import SetupOptionsDialog
from AnalyzeIndexDialog import AnalyzeIndexDialog
from AboutDialog import AboutDialog
from FundBasicInfoDialog import FundBasicInfoDialog
from FundNetValueDialog import FundNetValueDialog
from FundHoldDataDialog import FundHoldDataDialog
from TradeFundDialog import TradeFundDialog
from InitFundAccountDialog import InitFundAccountDialog

from utils.utils import format_convert


class MainWindow(QMainWindow, Ui_mainWindow):
    '''
    主窗口类
    '''

    def __init__(self):
        super(MainWindow, self).__init__()
        self.setupUi(self)

        self.userid = None
        self.current_fund = {"code": None, "name": None, "basic_info": None}

        self.setWindowTitle('个人基金管理 1.0 [未登录]')
        self.statusBar.showMessage('请先登录验证账户密码')
        self.setContentsMargins(5, 0, 5, 0)

        toolbar_spacer = QWidget()
        toolbar_spacer.setSizePolicy(
            QSizePolicy.Expanding, QSizePolicy.Preferred)
        self.toolBar.addWidget(toolbar_spacer)
        self.toolBar.addAction(self.actionExit)

        self.splitter = QSplitter(Qt.Horizontal, childrenCollapsible=False)
        self.splitter.addWidget(self.leftWidget)
        self.splitter.addWidget(self.rightWidget)
        self.setCentralWidget(self.splitter)

        self.fundNameLabel.setText('[未选择基金]')

        self.fundListWidget.setContextMenuPolicy(Qt.CustomContextMenu)
        self.fundListWidget.customContextMenuRequested.connect(self.onActionFundListWidgetContextMenu)
        self.fundListWidgetContextMenu = QMenu(self)
        self.fundListWidgetContextMenu.addAction(QAction(u'基本信息', self, triggered=self.onActionFundBasicInfoTriggered))
        self.fundListWidgetContextMenu.addAction(QAction(u'净值数据', self, triggered=self.onActionFundNetValueTriggered))
        self.fundListWidgetContextMenu.addAction(QAction(u'基金持仓', self, triggered=self.onActionFundHoldDataTriggered))

        self.anvTableWidget.setColumnCount(3)
        self.anvTableWidget.setHorizontalHeaderLabels(['日期', '单位净值', '累计净值'])
        self.anvTableWidget.setRowCount(0)
        self.anvTableWidget.horizontalHeader().setSectionResizeMode(QHeaderView.Stretch)

        self.tradeTableWidget.setColumnCount(4)
        self.tradeTableWidget.setHorizontalHeaderLabels(['日期', '交易', '数额', '标注'])
        self.tradeTableWidget.setRowCount(0)
        self.tradeTableWidget.horizontalHeader().setSectionResizeMode(QHeaderView.Stretch)
        self.tradeTableWidget.setContextMenuPolicy(Qt.CustomContextMenu)
        self.tradeTableWidget.customContextMenuRequested.connect(self.onActionTradeTableWidgetContextMenu)
        self.tradeTableWidgetContextMenu = QMenu(self)
        self.tradeTableWidgetContextMenu.addAction(QAction(u'买入交易', self, triggered=self.onActionTradeBuyTriggered))
        self.tradeTableWidgetContextMenu.addAction(QAction(u'卖出交易', self, triggered=self.onActionTradeSellTriggered))
        self.tradeTableWidgetContextMenu.addSeparator()
        self.tradeTableWidgetContextMenu.addAction(QAction(u'撤销交易', self, triggered=self.onActionTradeRevokeTriggered))
        self.tradeTableWidgetContextMenu.addAction(QAction(u'执行交易', self, triggered=self.onActionTradeExecuteTriggered))

        self.leftWidget.setEnabled(False)
        self.rightWidget.setEnabled(False)

    def loadFundAccountList(self):
        '''
        加载基金列表
        '''
        account_list = MySQLDB.getFundAccountList()
        if account_list:
            self.fundListWidget.clear()
            for item in account_list:
                list_item = QListWidgetItem(item['code'] + ' ' + item['name'])
                list_item.setData(Qt.UserRole, item)
                self.fundListWidget.addItem(list_item)
            self.fundListWidget.setCurrentRow(0)
        else:
            self.statusBar.showMessage('没有获取到基金列表')

    def setActionStatus(self, isLoggedIn):
        '''
        设置登录/注销后界面状态
        param: isLoggedIn: 登录状态
        '''
        self.actionLogin.setEnabled(not isLoggedIn)
        self.actionLogout.setEnabled(isLoggedIn)
        self.actionCreateFundAccount.setEnabled(isLoggedIn)
        self.actionRemoveFundAccount.setEnabled(isLoggedIn)

        self.actionExecuteTradeData.setEnabled(isLoggedIn)

        self.actionSetupFundAccount.setEnabled(isLoggedIn)
        self.actionSyncFundHistory.setEnabled(isLoggedIn)

        self.leftWidget.setEnabled(isLoggedIn)
        self.rightWidget.setEnabled(isLoggedIn)

        if isLoggedIn:
            self.setWindowTitle(f'个人基金管理 1.0 [{self.userid}]')
            self.statusBar.showMessage('已注销', 3000)

            self.statusBar.showMessage(f'{self.userid} 已登录', 3000)
        else:
            self.userid = None
            self.setWindowTitle('个人基金管理 1.0 [未登录]')
            self.statusBar.showMessage('请先登录验证账户密码')
            self.fundListWidget.clear()
            self.fundNameLabel.setText('[未选择基金]')

            self.fundHeldCostLabel.setText('0.00')
            self.fundHeldSharesLabel.setText('0.00')
            self.fundNVDateLabel.setText('N/A')
            self.fundNetValueLabel.setText('0.0000')
            self.fundADRatioLabel.setText('0.00%')
            self.fundADRatioLabel.setStyleSheet('font-size:20px; font-weight:bold;')
            self.fundTotalLabel.setText('0.00')
            self.fundProfitLabel.setText('0.00')
            self.fundProfitLabel.setStyleSheet('font-size:20px; font-weight:bold;')
            self.fundProfitRatioLabel.setText('0.00%')
            self.fundProfitRatioLabel.setStyleSheet('font-size:20px; font-weight:bold;')

            self.anvTableWidget.clearContents()
            self.anvTableWidget.setRowCount(0)
            self.tradeTableWidget.clearContents()
            self.tradeTableWidget.setRowCount(0)

    def closeEvent(self, event):
        '''
        窗口关闭事件
        '''
        if QMessageBox.question(self, '提示', "是否确定退出应用程序?", QMessageBox.Yes | QMessageBox.No, QMessageBox.No) == QMessageBox.Yes:
            event.accept()
        else:
            event.ignore()

    def onActionLoginTriggered(self):
        '''
        登录ACTION事件
        '''
        dialog = LoginDialog(self)
        if dialog.exec() == QDialog.Accepted:
            self.userid = dialog.userid
            self.setActionStatus(True)

            # 加载基金数据
            self.loadFundAccountList()

    def onActionLogoutTriggered(self):
        '''
        注销ACTION事件
        '''
        self.setActionStatus(False)

    def onActionCreateFundAccountTriggered(self):
        dialog = CreateFundAccountDialog(self)
        if dialog.exec() == QDialog.Accepted:
            self.loadFundAccountList()

    def onActionRemoveFundAccountTriggered(self):
        '''
        删除基金账户ACTION事件
        '''
        if self.current_fund is None:
            self.statusBar.showMessage('请先选择基金账户')
            return
        if QMessageBox.question(self, '提示', f"是否确定删除基金账户 {self.current_fund['code']} {self.current_fund['name']}？", QMessageBox.Yes | QMessageBox.No, QMessageBox.No) == QMessageBox.Yes:
            MySQLDB.deleteFundAccount(self.current_fund['code'])
            self.loadFundAccountList()
            self.statusBar.showMessage(f'已删除基金账户 {self.current_fund["code"]} {self.current_fund["name"]}')
            self.current_fund = None
            self.fundNameLabel.setText('[未选择基金]')
            self.anvTableWidget.clearContents()
            self.anvTableWidget.setRowCount(0)
            self.tradeTableWidget.clearContents()
            self.tradeTableWidget.setRowCount(0)

    def onActionExitTriggered(self):
        '''
        退出ACTION事件
        '''
        self.close()

    def onActionAnalyzeIndexTriggered(self):
        analyzeIndexDialog = AnalyzeIndexDialog(self)
        analyzeIndexDialog.exec()

    def onActionSetupFundAccountTriggered(self):
        dialog = SetupFundAccountDialog(self)
        if dialog.exec() == QDialog.Accepted:
            self.loadFundAccountList()

    def onActionSetupOptionsTriggered(self):
        dialog = SetupOptionsDialog(self)
        dialog.exec()

    def onActionSyncFundHistoryTriggered(self):
        dialog = SyncFundHistoryDialog(self)
        if dialog.exec() == QDialog.Accepted:
            self.loadFundAccountList()

    def onActionAboutTriggered(self):
        aboutDialog = AboutDialog(self)
        aboutDialog.exec()

    def onActionFundBasicInfoTriggered(self):
        dialog = FundBasicInfoDialog(parent=self, params=self.current_fund)
        dialog.exec()

    def onActionFundNetValueTriggered(self):
        dialog = FundNetValueDialog(parent=self, params=self.current_fund)
        dialog.loadData(6)
        dialog.exec()

    def onActionFundHoldDataTriggered(self):
        dialog = FundHoldDataDialog(parent=self, params=self.current_fund)
        dialog.loadData()
        dialog.exec()

    def onActionInitFundAccountTriggered(self):
        dialog = InitFundAccountDialog(parent=self, params=self.current_fund)
        if dialog.exec() == QDialog.Accepted:
            self.showFundHoldingData(self.current_fund["code"])
        else:
            self.statusBar.showMessage(f'[{self.current_fund["code"]}] {self.current_fund["name"]} 初始化基金持仓数据失败')

    def onActionFundListWidgetContextMenu(self, pos):
        '''
        基金列表项右键点击事件
        param: pos: 右键点击位置
        '''
        item = self.fundListWidget.itemAt(pos)
        if item is None:
            return
        self.fundListWidgetContextMenu.exec(self.fundListWidget.mapToGlobal(pos))

    def onFundListWidgetItemSelectionChanged(self):
        '''
        基金列表项选择事件
        '''
        item = self.fundListWidget.currentItem()
        if item is None:
            return
        item_data = item.data(Qt.UserRole)
        if item_data is None:
            return
        self.current_fund['code'] = item_data['code']
        self.current_fund['name'] = item_data['name']
        self.current_fund['basic_info'] = item_data['basic_info']

        self.fundNameLabel.setText(f'{self.current_fund["name"]} [{self.current_fund['code']}]')
        self.statusBar.showMessage(f'已选择 [{self.current_fund["code"]}] {self.current_fund["name"]}')
        self.showFundHoldingData(self.current_fund["code"])
        self.showFundNetValueData(self.current_fund["code"])
        self.showFundTradingData(self.current_fund["code"])

    def showFundHoldingData(self, code):
        '''
        显示基金持仓数据
        param: code: 基金代码
        '''
        self.fundHeldCostLabel.setText('0.00')
        self.fundHeldSharesLabel.setText('0.00')
        self.fundNVDateLabel.setText('N/A')
        self.fundNetValueLabel.setText('0.0000')
        self.fundADRatioLabel.setText('0.00%')
        self.fundADRatioLabel.setStyleSheet('font-size:20px; font-weight:bold;')
        self.fundTotalLabel.setText('0.00')
        self.fundProfitLabel.setText('0.00')
        self.fundProfitLabel.setStyleSheet('font-size:20px; font-weight:bold;')
        self.fundProfitRatioLabel.setText('0.00%')
        self.fundProfitRatioLabel.setStyleSheet('font-size:20px; font-weight:bold;')

        # 获取最新持仓数据
        data = MySQLDB.getFundLastestHoldingData(code)
        if data is None:
            return

        holding_cost = format_convert.float_to_decimal(data["cost"], 2)
        holding_share = format_convert.float_to_decimal(data["share"], 2)

        self.fundHeldCostLabel.setText(f'{holding_cost:,.2f}')
        self.fundHeldSharesLabel.setText(f'{holding_share:,.2f}')

        # 显示基金最新净值
        nv_data = MySQLDB.getFundLatestNetValue(code)
        if nv_data is None:
            return
        nv_date = nv_data["trade_date"]
        nv_value = format_convert.float_to_decimal(nv_data["asset_net_value"], 4)
        nv_growth = format_convert.float_to_decimal(nv_data["growth_rate"], 2)

        # 显示基金最新净值日期及涨跌幅
        self.fundNVDateLabel.setText(f'{nv_date}')
        self.fundNetValueLabel.setText(f'{nv_value:,.4f}')
        if nv_growth < 0:
            self.fundADRatioLabel.setStyleSheet('font-size:20px; font-weight:bold; color: green;')
        elif nv_growth > 0:
            self.fundADRatioLabel.setStyleSheet('font-size:20px; font-weight:bold; color: red;')
        else:
            self.fundADRatioLabel.setStyleSheet('font-size:20px; font-weight:bold;')
        self.fundADRatioLabel.setText(f'{nv_growth:,.2f}%')

        total_value = format_convert.float_to_decimal(nv_value * holding_share, 2)
        self.fundTotalLabel.setText(f'{total_value:,.2f}')

        profit_value = total_value - holding_cost
        self.fundProfitLabel.setText(f'{profit_value:,.2f}')
        if profit_value < 0:
            self.fundProfitLabel.setStyleSheet('font-size:20px; font-weight:bold; color: green;')
        elif profit_value > 0:
            self.fundProfitLabel.setStyleSheet('font-size:20px; font-weight:bold; color: red;')
        else:
            self.fundProfitLabel.setStyleSheet('font-size:20px; font-weight:bold;')

        if holding_cost == 0:
            profit_ratio = 0
        else:
            profit_ratio = format_convert.float_to_decimal(profit_value / holding_cost, 2)
        self.fundProfitRatioLabel.setText(f'{profit_ratio*100:,.2f}%')
        if profit_ratio < 0:
            self.fundProfitRatioLabel.setStyleSheet('font-size:20px; font-weight:bold; color: green;')
        elif profit_ratio > 0:
            self.fundProfitRatioLabel.setStyleSheet('font-size:20px; font-weight:bold; color: red;')
        else:
            self.fundProfitRatioLabel.setStyleSheet('font-size:20px; font-weight:bold;')

    def showFundNetValueData(self, code):
        if code is None:
            return
        # 从数据库加载基金持仓数据
        end_date = QDate.currentDate()
        start_date = end_date.addDays(-365)
        data = MySQLDB.getFundNetValue(code, start_date.toString('yyyy-MM-dd'), end_date.toString('yyyy-MM-dd'))
        if data is None:
            return

        # 填充基金持仓数据到表格
        self.anvTableWidget.clearContents()
        self.anvTableWidget.setRowCount(len(data))
        for row, item in enumerate(data):
            for col, value in enumerate(item.values()):
                table_item = QTableWidgetItem(str(value))
                table_item.setData(Qt.UserRole, item)
                table_item.setTextAlignment(Qt.AlignmentFlag.AlignCenter)
                self.anvTableWidget.setItem(row, col, table_item)

    def showFundTradingData(self, code):
        if code is None:
            return

        # 从数据库加载基金持仓数据
        data = MySQLDB.getFundTradingData(code)
        if data is None:
            return

        self.tradeTableWidget.clearContents()
        self.tradeTableWidget.setRowCount(len(data))
        for row, item in enumerate(data):
            for col, value in enumerate(item.values()):
                if col == 1:
                    if value == '买入':
                        value = '买入金额'
                    elif value == '卖出':
                        value = '卖出份额'
                elif col == 2:
                    value = format_convert.decimal_to_string(value, 2)
                table_item = QTableWidgetItem(str(value))
                table_item.setData(Qt.UserRole, item)
                table_item.setTextAlignment(Qt.AlignmentFlag.AlignCenter)
                self.tradeTableWidget.setItem(row, col, table_item)

    def onActionTradeTableWidgetContextMenu(self, pos):
        '''
        基金持仓表项右键点击事件
        param: pos: 右键点击位置
        '''
        table_item = self.tradeTableWidget.itemAt(pos)
        if table_item is None:
            self.tradeTableWidgetContextMenu.actions()[3].setEnabled(False)
            self.tradeTableWidgetContextMenu.actions()[4].setEnabled(False)
        else:
            item = table_item.data(Qt.UserRole)
            remark = item['remark']
            self.tradeTableWidgetContextMenu.actions()[3].setEnabled(remark == '未执行')
            self.tradeTableWidgetContextMenu.actions()[4].setEnabled(remark == '未执行')

        self.tradeTableWidgetContextMenu.exec(self.tradeTableWidget.mapToGlobal(pos))

    def onActionTradeBuyTriggered(self):
        '''
        买入交易ACTION事件
        '''
        dialog = TradeFundDialog(parent=self, params=self.current_fund, mode='买入')
        if dialog.exec() == QDialog.Accepted:
            self.showFundTradingData(self.current_fund['code'])

    def onActionTradeSellTriggered(self):
        '''
        卖出交易ACTION事件
        '''
        dialog = TradeFundDialog(parent=self, params=self.current_fund, mode='卖出')
        if dialog.exec() == QDialog.Accepted:
            self.showFundTradingData(self.current_fund['code'])

    def onActionTradeRevokeTriggered(self):
        '''
        撤销交易ACTION事件
        '''
        code = self.current_fund['code']
        name = self.current_fund['name']

        table_item = self.tradeTableWidget.currentItem()
        if table_item is None:
            return
        item = table_item.data(Qt.UserRole)
        trade_date = item['trade_date']
        if QMessageBox.question(self, '提示', f"是否确定撤销 [{code}] {name} {trade_date} 的交易记录?", QMessageBox.Yes | QMessageBox.No, QMessageBox.No) == QMessageBox.Yes:
            MySQLDB.deleteFundTradingData(code, trade_date)
            self.showFundTradingData(code)

    def onActionTradeExecuteTriggered(self):
        '''
        执行交易ACTION事件
        '''
        code = self.current_fund['code']
        name = self.current_fund['name']

        table_item = self.tradeTableWidget.currentItem()
        if table_item is None:
            return
        item = table_item.data(Qt.UserRole)
        trade_date = item['trade_date']
        if QMessageBox.question(self, '提示', f"是否确定执行 [{code}] {name} {trade_date} 的交易记录?", QMessageBox.Yes | QMessageBox.No, QMessageBox.No) == QMessageBox.Yes:
            if MySQLDB.executeFundTradingData(code, trade_date) > 0:
                self.showFundTradingData(code)
                self.showFundHoldingData(code)
